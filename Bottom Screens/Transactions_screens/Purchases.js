...
export const Purchases = (props) => {
    ...
    let total = 0
        for(let i = 0; i<transactions.length; i++){
            if(transactions[i]['paid'] == 'True'){
                total += transactions[i]['grandtotal']
            }
        }
        setgrandtotal(total)
  ...
  const getTransactions = (day , month , year) => {
    axios({
        method : 'GET',
        url : 'http://192.168.43.189:8000/transactions/Purchases/' + day + '/' + month + '/' + year + '/',
        data : {
  ...
  const getTransactiondetails = (id) => {
    axios({
        method : 'GET',
        url : 'http://192.168.43.189:8000/transactionDetails/' + id + '/',
        data : {
  ...
  return (
    <View  style = {styles.container} >
        ...
        <Avatar
            ...
            onPress = {()=>{
                alert('Total purchases created amount to UGX ' + numberWithCommas(grandtotal) + ' so far.')
            }}/>
            <Avatar  
                size = {48}
                rounded
...
<ScrollView horizontal = {true}>
        <ScrollView  style = {{ width : 2.8* ScreenWidth }}>
          <DataTable
              ...
              colSettings={[
                ...
                {name: 'discount', type: COL_TYPES.INT, width: '9.2%'},
                {name: 'added_by', type: COL_TYPES.INT, width: '9.2%'},
        ...
        <Overlay isVisible = {isVisible} onBackdropPress={()=>{setIsVisible(false)}} >
                    <View style = {styles.overlay} >
                        <Text style = {{ fontSize : 15 , fontWeight :'bold' }}>Transaction overview for {activetransaction ? (activetransaction['dea_cust_name']) : ('')}</Text>
                        <ScrollView>
                        <ScrollView horizontal = {true} >
                            <DataTable                   
                            data={activetransactiondetails ? (activetransactiondetails) : ([])}
                            colNames={['product_name', 'qty', 'rate' , 'total', 'profit']} //List of Strings
                            colSettings={[
                                { name: 'product_name', type: COL_TYPES.STRING, width: '30%' }, 
                                {name: 'qty', type: COL_TYPES.STRING, width: '10%'},
                                {name: 'rate', type: COL_TYPES.STRING, width: '20%'},
                                {name: 'total', type: COL_TYPES.INT, width: '20%'},
                                {name: 'profit', type: COL_TYPES.INT, width: '20%'},
